...
def productos(request, accion, id):
    ...
    if request.method == 'POST':
        ...
        if accion == 'crear':
            ...
            form = ProductoForm(request.POST, request.FILES, instance=Producto.objects)
    ...
    if request.method == 'GET':
        if accion == 'crear': form = ProductoForm()
...
def usuarios(request, accion, id):
    ...
    if request.method == 'POST':
        ...
        form_usuario = UsuarioForm(request.POST, instance=perfil)
        ...
        if form_usuario.is_valid() and form_perfil.is_valid():
            ...
            perfil = form_perfil.save(commit=False)
            usuario.save()
            messages.success(request, f'El usuario {usuario.first_name}{usuario.last_name} fue creado con exito')
...
def bodega(request):
    ...
    if request.method == 'POST':
        ...
        if cantidad == 1:
            ...
            messages.success(request, f'Se ha agrega {cantidad} productos de "{producto.nombre}" a la bodega')
...
def enviar_correo_cambio_password(request, user, password):
    try:
        ...
        subject = 'Cambio de contraseña PixelPlayer'
